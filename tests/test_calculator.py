...
def test_start_stop(calculator):
    ...
    with patch('threading.Thread') as MockThread, \
         ...
         patch('pynput.keyboard.Listener') as MockKeyboard:
...
def test_update_loop(calculator):
    ...
    # Mock time.sleep to avoid waiting but allow context switch if needed
    with patch('time.sleep'):
        # Create a thread to run the loop
        t = threading.Thread(target=calculator._update_loop)
        t.start()
        
        # Let it run "a bit" (in mock time)
        # Since sleep is mocked, it will loop fast. We just need 1 call.
        # We set stop event quickly from main thread
        time.sleep(0.1) # Real wait to let thread start
